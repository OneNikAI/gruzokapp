...
const phoneToEmail = (phone: string) => `${normalizePhone(phone)}@gruzok.ru`;
...
export const checkUserExists = async (phone: string) => {
  const path = 'users';
  try {
    const q = query(collection(db, path), where('phone', '==', normalizePhone(phone)));
...
export const authService = {
  register: async (phone: string, name: string, role: string, password: string) => {
    try {
  ...
  sendVerificationCode: async (phone: string) => {
    ...
    const path = `verification_codes/${phone}`;
    try {
      ...
      console.warn("Firestore write failed for verification code, returning code for demo:", error);
      // Return code anyway so it can be shown in the UI for demo/test
      return code;
    }
  },

  verifyCode: async (phone: string, inputCode: string) => {
    const path = `verification_codes/${phone}`;
    try {
      const docSnap = await getDoc(doc(db, 'verification_codes', phone));
      if (docSnap.exists()) {
      ...
      console.error("Error verifying code from Firestore:", error);
      return false;
    }
  }
};
